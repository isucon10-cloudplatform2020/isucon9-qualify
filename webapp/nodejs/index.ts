...
async function getNewCategoryItems(
  req: FastifyRequest,
  reply: FastifyReply<ServerResponse>
) {
...
async function getItem(
  ...
  if (
    (user.id === item.seller_id || user.id === item.buyer_id) &&
    item.buyer_id !== 0
  ) {
    const buyer = await getUserSimpleByID(db, item.buyer_id);
    if (buyer === null) {
...
async function postItemEdit(
  ...
  await db.query("SELECT * FROM `items` WHERE `id` = ? FOR UPDATE", [
    targetItem.id,
  ]);
...
async function postBuy(
  ...
  let seller: User | null = null;
  {
    const [
      rows,
    ] = await db.query("SELECT * FROM `users` WHERE `id` = ? FOR UPDATE", [
  ...
  await db.query(
    ...
    [buyer.id, ItemStatusTrading, new Date(), targetItem.id]
  );
...
async function postSell(
  ...
  await fs.promises.writeFile(
    `../public/upload/${imgName}`,
    req.body.image[0].data
  );
...
async function postShip(
  ...
  await db.query(
    ...
    [ShippingsStatusWaitPickup, img, new Date(), transactionalEvidence.id]
  );
...
async function postShipDone(
  ...
  try {
    ...
    if (
      !(
        res.status === ShippingsStatusShipping ||
        res.status === ShippingsStatusDone
      )
    ) {
      replyError(
        reply,
        "shipment service側で配送中か配送完了になっていません",
        403
      );
      await db.rollback();
      await db.release();
    ...
    await db.query(
      ...
      [res.status, new Date(), transactionEvidence.id]
    );
  } catch (res) {
    replyError(reply, "failed to request to shipment service");
    await db.rollback();
  ...
  await db.query(
    ...
    [TransactionEvidenceStatusWaitDone, new Date(), transactionEvidence.id]
  );
...
async function getQRCode(
  ...
  let transactionEvidence: TransactionEvidence | null = null;
  {
    const [
      rows,
    ] = await db.query("SELECT * FROM `transaction_evidences` WHERE `id` = ?", [
      transactionEvidenceId,
    ]);
    for (const row of rows) {
      transactionEvidence = row as TransactionEvidence;
  ...
  if (
    ...
    shipping.status !== ShippingsStatusShipping
  ) {
  ...
  reply.code(200).type("image/png").send(shipping.img_binary);
}

async function postBump(
  req: FastifyRequest,
  reply: FastifyReply<ServerResponse>
) {
  const csrfToken = req.body.csrf_token;
  ...
  await db.query(
    ...
    [now, now, targetItem.id]
  );

  await db.query("UPDATE `users` SET `last_bump`=? WHERE id=?", [
    now,
    seller.id,
  ]);
...
async function postLogin(
  ...
  const [
    rows,
  ] = await db.query("SELECT * FROM `users` WHERE `account_name` = ?", [
    accountName,
  ]);
  ...
  reply.code(200).type("application/json;charset=utf-8").send(user);
}
...
async function postRegister(
  ...
  const [
    ...
    [accountName, hashedPassword, address]
  );
...
async function getReports(
  ...
  reply.code(200).type("application/json").send(transactionEvidences);
}
...
async function getUserSimpleByID(
  db: MySQLQueryable,
  userID: number
